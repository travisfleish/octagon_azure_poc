#!/usr/bin/env python3
"""
Minimal vector indexing script that avoids collection field issues.
"""
...
async def create_minimal_index():
    """Create a minimal search index with only basic fields."""
    try:
        vector_service = VectorService()
        
        # Create a minimal index schema
        ...
        search_index = SearchIndex(
            name="octagon-sows-minimal",
            fields=fields,
            vector_search=vector_search,
        )
        
        # Update the index name in the service
        vector_service._index_name = "octagon-sows-minimal"
        
        # Create the index
        index_client = await vector_service._get_index_client()
        await index_client.create_or_update_index(search_index)
        print("✅ Created minimal search index: octagon-sows-minimal")
        
        return True
        
    except Exception as e:
        print(f"❌ Failed to create minimal index: {e}")
        return False


async def index_text_files_minimal():
    """Index text files with the minimal schema."""
    try:
        vector_service = VectorService()
        vector_service._index_name = "octagon-sows-minimal"
...
async def main():
    print("Creating minimal vector index...")
    success = await create_minimal_index()
    
    if success:
        print("\nIndexing text files...")
        await index_text_files_minimal()
    else:
